    ...
    >
      ...
      {editing ? (
        <form onSubmit={handleSubmit} className="flex-1 flex items-center gap-2">
          <input
            type="text"
            className="flex-1 bg-bolt-elements-bg-depth-1 text-bolt-elements-textPrimary rounded-md px-3 py-1.5 text-sm border border-bolt-elements-borderColor focus:outline-none focus:ring-1 focus:ring-bolt-elements-borderColorActive"
            autoFocus
          ...
          <button
            type="submit"
            className="i-ph:check h-4 w-4 text-bolt-elements-textSecondary hover:text-bolt-elements-item-contentAccent transition-colors"
            onMouseDown={handleSubmit}
          />
        </form>
      ) : (
        <a
          href={createChatUrl(item.urlId || '')}
        ...
        >
          ...
          >
            <div className="flex items-center gap-2.5 text-bolt-elements-textSecondary opacity-0 group-hover:opacity-100 transition-opacity">
              <ChatActionButton
    ...
    return (
      <WithTooltip tooltip={toolTipContent} position="bottom" sideOffset={4}>
        <button
          ref={ref}
          type="button"
          className={`text-bolt-elements-textSecondary hover:text-bolt-elements-item-contentAccent transition-colors ${icon} ${className ? className : ''}`}
          onClick={onClick}
        />
      </WithTooltip>
    );
